...
export const Route = createFileRoute("/tools/$slug")({
  ...
  notFoundComponent: () => (
    <div className="max-w-3xl mx-auto px-4 py-24 text-center">
      ...
      <Link to="/tools" className="btn-cyber">← Back to tools</Link>
    </div>
  ),
...
function ToolDetail() {
  ...
  return (
    <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Link to="/tools" className="font-mono-cyber text-xs text-accent hover:underline">← All tools</Link>
      ...
      <header className="mt-6 mb-10 flex flex-wrap items-start gap-6">
        ...
        <div className="flex-1 min-w-[260px]">
          ...
          <h1 className="font-display font-black text-3xl sm:text-4xl text-cyber-white leading-tight">{tool.name}</h1>
          <p className="mt-2 font-mono-cyber text-sm text-muted-foreground">Provided by {tool.provider}</p>
          <p className="mt-4 text-cyber-text">{tool.about}</p>
          <div className="mt-5 flex flex-wrap gap-2">
            <a href={tool.url} target="_blank" rel="noreferrer noopener" className="btn-cyber !py-2 !px-4 !text-xs">
              ↗ Open {tool.provider}
            </a>
            <a
      ...
      <section className="mt-10 card-cyber rounded p-6">
        ...
        <p className="text-sm text-cyber-text">
          Cyber Threat Watch is not affiliated with {tool.provider}. We curate this directory to help defenders find
          reputable, free platforms. Always validate downloads with checksums, use official URLs, and review the
          provider's privacy policy before submitting sensitive data.
        </p>
      </section>

      {related.length > 0 && (
        <section className="mt-12">
          <h2 className="font-display font-bold text-lg text-cyber-white mb-4">More in {tool.category}</h2>
